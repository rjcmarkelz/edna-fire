...
df_97 %>%
  ...
  filter(row_number() == 1) %>%
  ungroup() %>%
  filter(percent >= 99) # 110 / 405

df_97 %>%
  group_by(otu) %>%
  filter(row_number() == 1) %>%
  ungroup() %>%
  filter(percent > 97) # 163 / 405

length(unique(df_97$asv))
...
table <- kable(family_taxonomy_table, format = "html", 
               col.names = c("Family", "Burned sites", "Unburned sites")) %>%
  kable_styling(full_width = FALSE) %>%
  row_spec(0, bold = TRUE) 

kable_styling(table, html_font = "helvatica") %>%
  save_kable("supp_taxonomy.pdf")
...
diversity_shannon <- 
  as.data.frame(vegan::diversity(hell_mat, index = "shannon")) %>%
    rownames_to_column(var = "id")
colnames(diversity_shannon) <- c("sample_name", "div")

diversity_simpson <- 
  as.data.frame(vegan::diversity(hell_mat, index = "simpson")) %>%
    rownames_to_column(var = "id")
colnames(diversity_simpson) <- c("sample_name", "div")

diversity_rich <- 
  as.data.frame(BAT::alpha(hell_mat)) %>%
  rownames_to_column(var = "id")
...
nmds_supp <- ggarrange(nmdseuc, nmdsjacc, 
                       labels = c("Hellinger Standardized", "Incidence"),
                       common.legend = TRUE)
...
nmds_euc <- 
  ...
  labs(color = "Burn Status", 
       shape = "Burn Status",
       lty = "Burn Status")
ggsave("nmds_burn.png", width = 7, height = 5, units = "in", bg = "white")
