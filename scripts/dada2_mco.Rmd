...
setwd("D:/Research/rprojects/fire_project")
...
# If R1 == R2, continue on to the following steps
  ...
  sample_names <- sapply(str_split(basename(files_fwd), "_L001"), '[', 1)
...
filter_out <- filterAndTrim(files_fwd, filt_fwd, files_rev, filt_rev,
              ...
              minQ = 3) # Save results to calculate reads through pipeline when completed
...
save.image("mco_dada2.RData")
```

Edit produced tables
```{r}
# Remove the file name
name_split <- sub("_F_filt_mco.fastq.gz$", "",
...
# Trim length of sequences to expected size
  ...
  table_final <- table_final[,nchar(colnames(table_final)) %in% 312:313]
